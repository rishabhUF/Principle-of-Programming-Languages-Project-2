...
public class ScannerTest {
    ...
    public ExpectedException thrown = ExpectedException.none();
        ...
        public void testEmpty() throws IllegalCharException, IllegalNumberException {
                ...
                {
                	System.out.println(t.kind+ " "+ t.pos);
                }
        }
...
//                    System.out.println(scanner);
//            }
              checkNext(scanner, KW_IF, 0);
              checkNext(scanner, KW_WHILE, 3);
      }
...
//            }
     ...
     @Test
  	public void testMultKeywords() throws IllegalCharException, IllegalNumberException {
  		//input string
  		String input = "while     if";
  		//create and initialize the scanner
  		Scanner scanner = new Scanner(input);
  		scanner.scan();
  		//get the first token and check its kind, position, and contents
  		...
  		assertEquals(0, token.pos);
  		String text = KW_WHILE.getText();
  		assertEquals(text.length(), token.length);
  		assertEquals(text, token.getText());
  		
  		Scanner.Token token2 = scanner.nextToken();
  		assertEquals(Scanner.Kind.KW_IF,token2.kind);
  		
  		assertEquals(10, token2.pos);
  		String text2 = KW_IF.getText();
...
//              }
        ...
        @Test
        public void test19a() throws IllegalCharException, IllegalNumberException{
                String input="123 456";
                Scanner scanner = new Scanner(input);
                scanner.scan();
//              if (doPrint) {
...
//              }
                checkNext(scanner, INT_LIT, 0, "123");
                checkNext(scanner, INT_LIT, 4, "456");
